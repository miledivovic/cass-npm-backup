let EcRsaOaep = require("./EcRsaOaep.js");
let EcAes = require("./EcAes.js");
let EcCrypto = require("./EcCrypto.js");
let EcPpk = require("./EcPpk.js");
let chai = require("chai");

let hrtime = function() {
    try {
        return [Math.round(performance.now()/1000), performance.now() * 1000];
    } catch (e) {
        // Eat quietly.
    }
    return process.hrtime();
};

var should = chai.should();
var expect = chai.expect;
var assert = chai.assert;

describe("EcRsaOaep", () => {
    it('encryption then decryption', () => {
        var randomString = EcAes.newIv(256).substring(0, 190);
        var ppk = EcPpk.fromPem(
            "-----BEGIN RSA PRIVATE KEY----------END RSA PRIVATE KEY-----"
        );
        var pk = ppk.toPk();
        var encrypted = EcRsaOaep.encrypt(pk, randomString);
        var decrypted = EcRsaOaep.decrypt(ppk, encrypted);
        assert.isTrue(randomString == decrypted);
    });
    it('encryption then decryption (utf-8)', () => {
        var randomString =
            "abc\u16a0\u16c7\u16bb\u16eb\u16d2\u16e6\u16a6\u16eb\u16a0\u16b1\u16a9\u16a0\u16a2\u16b1\u16eb\u16a0\u16c1\u16b1\u16aa\u16eb\u16b7\u16d6\u16bb\u16b9\u16e6\u16da\u16b3\u16a2\u16d7";
        var ppk = EcPpk.fromPem(
            "-----BEGIN RSA PRIVATE KEY----------END RSA PRIVATE KEY-----"
        );
        var pk = ppk.toPk();
        var encrypted = EcRsaOaep.encrypt(pk, randomString);
        var decrypted = EcRsaOaep.decrypt(ppk, encrypted);
        assert.isTrue(randomString == decrypted);
    });
    it('encryption then decryption w/caching', () => {
        var randomString = EcAes.newIv(256).substring(0, 190);
        var ppk = EcPpk.fromPem(
            "-----BEGIN RSA PRIVATE KEY----------END RSA PRIVATE KEY-----"
        );
        var pk = ppk.toPk();
        var hrTime = hrtime();
        let encrypted = EcRsaOaep.encrypt(pk, randomString);
        let elapsed = (hrtime()[0]*1000000 + hrtime()[1]/1000 - hrTime[0] * 1000000 - hrTime[1] / 1000)/1000;
        console.log(randomString.length/1024+"KB encryption speed: " + elapsed+"ms");
        hrTime = hrtime();
        EcCrypto.caching = true;
        let decrypted = EcRsaOaep.decrypt(ppk, encrypted);
        elapsed = (hrtime()[0]*1000000 + hrtime()[1]/1000 - hrTime[0] * 1000000 - hrTime[1] / 1000)/1000;
        console.log("decryption wout/caching speed: " + elapsed+"ms");
        hrTime = hrtime();
        decrypted = null;
        decrypted = EcRsaOaep.decrypt(ppk, encrypted);
        elapsed = (hrtime()[0]*1000000 + hrtime()[1]/1000 - hrTime[0] * 1000000 - hrTime[1] / 1000)/1000;
        console.log("decryption w/caching speed: " + elapsed+"ms");
        assert.isTrue(elapsed < 1);
        assert.isTrue(randomString == decrypted);
    });
    it('signing then verifying', () => {
        var randomString = EcAes.newIv(256*4);
        var ppk = EcPpk.fromPem(
            "-----BEGIN RSA PRIVATE KEY----------END RSA PRIVATE KEY-----"
        );
        var pk = ppk.toPk();
        var signature = EcRsaOaep.sign(ppk, randomString);
        var verified = EcRsaOaep.verify(pk, randomString, signature);
        assert.isTrue(verified);
    });
    it('signing then verifying (utf-8)', () => {
        var randomString =
            "abc\u16a0\u16c7\u16bb\u16eb\u16d2\u16e6\u16a6\u16eb\u16a0\u16b1\u16a9\u16a0\u16a2\u16b1\u16eb\u16a0\u16c1\u16b1\u16aa\u16eb\u16b7\u16d6\u16bb\u16b9\u16e6\u16da\u16b3\u16a2\u16d7";
        var ppk = EcPpk.fromPem(
            "-----BEGIN RSA PRIVATE KEY----------END RSA PRIVATE KEY-----"
        );
        var pk = ppk.toPk();
        var signature = EcRsaOaep.sign(ppk, randomString);
        var verified = EcRsaOaep.verify(pk, randomString, signature);
        assert.isTrue(verified);
    });
    it('signing then verifying w/caching', () => {
        var randomString = EcAes.newIv(256).substring(0, 190);
        var ppk = EcPpk.fromPem(
            "-----BEGIN RSA PRIVATE KEY----------END RSA PRIVATE KEY-----"
        );
        var pk = ppk.toPk();
        var hrTime = hrtime();
        var signature = EcRsaOaep.sign(ppk, randomString);
        let elapsed = (hrtime()[0]*1000000 + hrtime()[1]/1000 - hrTime[0] * 1000000 - hrTime[1] / 1000)/1000;
        console.log(randomString.length/1024+"KB signing speed: " + elapsed+"ms");
        hrTime = hrtime();
        EcCrypto.caching = true;
        let verified = EcRsaOaep.verify(pk, randomString, signature);
        elapsed = (hrtime()[0]*1000000 + hrtime()[1]/1000 - hrTime[0] * 1000000 - hrTime[1] / 1000)/1000;
        console.log("verification wout/caching speed: " + elapsed+"ms");
        hrTime = hrtime();
        verified = EcRsaOaep.verify(pk, randomString, signature);
        elapsed = (hrtime()[0]*1000000 + hrtime()[1]/1000 - hrTime[0] * 1000000 - hrTime[1] / 1000)/1000;
        console.log("verification w/caching speed: " + elapsed+"ms");
        assert.isTrue(verified);
    });
    it('signing then verifying (sha256)', () => {
        var randomString = EcAes.newIv(256*4);
        var ppk = EcPpk.fromPem(
            "-----BEGIN RSA PRIVATE KEY----------END RSA PRIVATE KEY-----"
        );
        var pk = ppk.toPk();
        var signature = EcRsaOaep.signSha256(ppk, randomString);
        var verified = EcRsaOaep.verifySha256(pk, randomString, signature);
        assert.isTrue(verified);
    });
    it('signing then verifying (utf-8) (sha256)', () => {
        var randomString =
            "abc\u16a0\u16c7\u16bb\u16eb\u16d2\u16e6\u16a6\u16eb\u16a0\u16b1\u16a9\u16a0\u16a2\u16b1\u16eb\u16a0\u16c1\u16b1\u16aa\u16eb\u16b7\u16d6\u16bb\u16b9\u16e6\u16da\u16b3\u16a2\u16d7";
        var ppk = EcPpk.fromPem(
            "-----BEGIN RSA PRIVATE KEY----------END RSA PRIVATE KEY-----"
        );
        var pk = ppk.toPk();
        var signature = EcRsaOaep.signSha256(ppk, randomString);
        var verified = EcRsaOaep.verifySha256(pk, randomString, signature);
        assert.isTrue(verified);
    });
    it('signing then verifying w/caching (sha256)', () => {
        var randomString = EcAes.newIv(256).substring(0, 190);
        var ppk = EcPpk.fromPem(
            "-----BEGIN RSA PRIVATE KEY----------END RSA PRIVATE KEY-----"
        );
        var pk = ppk.toPk();
        var hrTime = hrtime();
        var signature = EcRsaOaep.signSha256(ppk, randomString);
        let elapsed = (hrtime()[0]*1000000 + hrtime()[1]/1000 - hrTime[0] * 1000000 - hrTime[1] / 1000)/1000;
        console.log(randomString.length/1024+"KB signing speed: " + elapsed+"ms");
        hrTime = hrtime();
        EcCrypto.caching = true;
        let verified = EcRsaOaep.verifySha256(pk, randomString, signature);
        elapsed = (hrtime()[0]*1000000 + hrtime()[1]/1000 - hrTime[0] * 1000000 - hrTime[1] / 1000)/1000;
        console.log("verification wout/caching speed: " + elapsed+"ms");
        hrTime = hrtime();
        verified = EcRsaOaep.verifySha256(pk, randomString, signature);
        elapsed = (hrtime()[0]*1000000 + hrtime()[1]/1000 - hrTime[0] * 1000000 - hrTime[1] / 1000)/1000;
        console.log("verification w/caching speed: " + elapsed+"ms");
         assert.isTrue(verified);
    });
});
...
describe("EcRsaOaepAsyncWorker", () => {
    ...
    it('signing then verifying', async () => {
        ...
        var verified = await EcRsaOaepAsyncWorker.verify(pk, randomString, signature);
    ...
    it('signing then verifying (sha256)', async () => {
        ...
        var verified = await EcRsaOaepAsyncWorker.verifySha256(pk, randomString, signature);
        assert.isTrue(verified);
    });
    ...
    it('signing then verifying w/caching (sha256)', async () => {
        ...
        hrTime = hrtime();
        EcCrypto.caching = true;
        let verified = await EcRsaOaepAsyncWorker.verifySha256(pk, randomString, signature);
        elapsed = (hrtime()[0]*1000000 + hrtime()[1]/1000 - hrTime[0] * 1000000 - hrTime[1] / 1000)/1000;
        console.log("verification wout/caching speed: " + elapsed+"ms");
        hrTime = hrtime();
